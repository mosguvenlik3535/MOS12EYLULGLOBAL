...
/** Cash ledger balance through now, split at the local calendar month boundary.
...
export function monthlyCash(state: Pick<AppState, 'sales' | 'expenses' | 'cashMoves' | 'customers' | 'invoices'>, now = new Date()) {
  ...
  let carry = 0, incoming = 0, outgoing = 0;
  ...
  state.sales.forEach(s => add(s.date, s.cash || 0)); // refunds already have negative cash
  ...
  state.invoices.forEach(inv => {
    if (inv.payments?.length) {
      inv.payments.filter(p => p.method === 'nakit').forEach(p => add(p.date, -p.amount));
    } else if (inv.status === 'odendi' && inv.payType === 'nakit') {
      add(inv.paidDate || inv.date, -inv.total);
    }
  });
  return { carry: round2(carry), incoming: round2(incoming), outgoing: round2(outgoing), balance: round2(carry + incoming - outgoing) };
}
